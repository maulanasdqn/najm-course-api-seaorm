...
pub async fn mutation_create_permission(payload: Json<PermissionsRequestDto>) -> Response {
    ...
    match new_permission.insert(&db).await {
        Ok(_) => common_response(StatusCode::CREATED, "Permission created successfully"),
...
pub async fn query_get_permissions(params: MetaRequestDto) -> Response {
    ...
    if !search.is_empty() {
        query = query.filter(Expr::col(PermissionsColumn::Name).ilike(format!("%{}%", search)));
    }
...
pub async fn query_get_permission_by_id(id: String) -> Response {
        ...
        .await
    {
        ...
        Ok(None) => return common_response(StatusCode::NOT_FOUND, "Permission not found"),
        Err(err) => return common_response(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string()),
    };
...
pub async fn mutation_update_permission(
    ...
    let permission_id = match Uuid::parse_str(&id) {
        Ok(id) => id,
        Err(_) => return common_response(StatusCode::BAD_REQUEST, "Invalid permission ID format"),
    };
